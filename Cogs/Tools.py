...
class Tools(commands.Cog):
    ...
    async def on_ready(self):
        ...
        for guild in self.bot.guilds:
            ...
            self.default_channels[guild.id] = temp_default_channel.id
    ...
    def is_command_channel(self, command: Command):
        return isinstance(command.get_channel(), discord.channel.DMChannel) or command.get_channel().id == self.default_channels[command.get_guild().id]
    ...
    async def command_event(self, context: commands.Context):
        ...
        event_type = Util.EventType.EventUnknown

        if input_event_string == 'earthquakes_pacific':
            event_type = Util.EventType.EarthquakePacific
            
        elif input_event_string == 'earthquakes_global':
            event_type = Util.EventType.EarthquakeGlobal

        # final check before performing the action
        action_string = command.get_part(1)
        if not action_string in ['sub', 'unsub'] or event_type == Util.EventType.EventUnknown:
            return
        
        # find the file to append this channel id to
        file_path = fr'{Tools.EVENT_SUB_PATH}{event_type.value}.json'
    ...
    async def on_earthquake_global_watcher(self):
        ...
        file_path = fr'{Tools.EVENT_SUB_PATH}{Util.EventType.EarthquakeGlobal.value}.json'
        await Tools.generic_earthquake_watcher('Global', file_path, None, None, None, 6.0)


    @commands.Cog.listener()
    async def on_earthquake_pacific_watcher(self):
        print('now watching for new pacific earthquake events')
        file_path = fr'{Tools.EVENT_SUB_PATH}{Util.EventType.EarthquakePacific.value}.json'
        await Tools.generic_earthquake_watcher('Pacific', file_path, 45.44683044, -122.77973641, 200, 3.5)
    ...
    async def generic_earthquake_watcher(earthquake_display_string, file_path, latt, long, radius, min_mag):
        while True:
            ...
            this_iteration_contents = await Tools.load_subscriber_file(file_path, this_iteration_contents)
            if not this_iteration_contents:
                await asyncio.sleep(Tools.EVENT_POLL_RATE)
                continue
            ...
            new_quakes = await Tools.get_earthquakes_since_time(this_iteration_contents['last_checked'], latt, long, radius, min_mag)
            this_iteration_contents['last_checked'] = now

            # write last check time to file
            result = await Tools.write_subscriber_file(file_path, this_iteration_contents)
            if not result:
                await asyncio.sleep(Tools.EVENT_POLL_RATE)
                continue
