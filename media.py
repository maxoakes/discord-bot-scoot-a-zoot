...
async def command_stream(context: commands.Context):
    ...
    if source_string == "" or (source_string.find('http') != 0 and source_string.find('file') != 0):
        await default_channel.send(embed=Util.create_simple_embed(f"Bad input. Must be an `http://`, `https://`, or `file://` protocol, or see the `presets` command", MessageType.NEGATIVE))
        return
    if source_string:
        await default_channel.send(f"**Acquiring metadata for `{source_string[:64]}`...**")
